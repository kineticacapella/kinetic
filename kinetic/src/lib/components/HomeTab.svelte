<script lang="ts">
  ...
  let workoutSearchText = $state('');
  let filteredWorkouts = $derived(
    ($workouts || []).filter(
        (w: Workout) => w.id && w.name.toLowerCase().includes(workoutSearchText.toLowerCase())
    )
  );
                    ...
                    <div bind:this={scrollContainer} onscroll={handleScroll} class="flex overflow-x-auto space-x-4 p-2 scrollbar-hide">
                        {#if currentWeekDates.length > 0}
                            {#each microcycle.days as day (day.dayIndex)}
                                ...
                                <div class="relative flex-shrink-0 w-[455px] h-64 p-4 rounded-lg bg-gray-50 dark:bg-gray-700 flex flex-col {borderClass}">
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-lg shadow-xl dark:bg-gray-800 border-2 border-blue-700 dark:border-blue-600">
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                ...
                <button type="button" class="text-red-500 bg-transparent hover:bg-red-100 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:text-red-400 dark:hover:bg-red-900" onclick={() => workoutModal.hide()}>
                    <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                    </svg>
                    <span class="sr-only">Close modal</span>
            ...
            <div class="p-4 md:p-5">
                <div class="mb-4">
                    <label for="workout-search" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Search</label>
                    <input type="text" id="workout-search" bind:value={workoutSearchText} class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" placeholder="Search for a workout">
                </div>
                <div class="flex flex-col gap-2 max-h-60 overflow-y-auto rounded-lg border border-gray-300 dark:border-gray-600 p-2">
                    {#each filteredWorkouts as workout (workout.id)}
                        <button type="button" onclick={() => assignWorkout(workout.id as string)} class="text-left w-full text-sm font-medium text-gray-900 dark:text-white p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 border border-gray-200 dark:border-gray-700">
                            {workout.name}
                        </button>
                    {/each}
                </div>
